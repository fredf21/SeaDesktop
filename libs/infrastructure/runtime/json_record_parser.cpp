...
DynamicRecord JsonRecordParser::parse(const sea::domain::Entity& entity,
    ...
    try {
        ...
        throw sea::sea_errors_handling::RUNTIME_EXECUTION(std::string("[RUNTIME EXECUTION] JSON invalide: ") + e.what());
    }
    ...
    for (const auto& field : entity.fields) {
        try{
            ...
            case sea::domain::FieldType::Decimal:{
                if (!value.is_string()) {
                    throw sea::sea_errors_handling::RUNTIME_EXECUTION("[RUNTIME EXECUTION] Le champ '" + field.name + "' doit etre une chaine.");
                }
            ...
            case sea::domain::FieldType::BigInt:
            {
                if (!value.is_number_integer()) {
                    throw sea::sea_errors_handling::RUNTIME_EXECUTION("[RUNTIME EXECUTION] Le champ '" + field.name + "' doit etre un entier.");
                }
                if(unsignedvalue)
...

            {
                ...
                if(unsignedvalue){
                    if(value.get<std::uint16_t>() < std::numeric_limits<std::uint16_t>::min() ||  value.get<std::uint16_t>() > std::numeric_limits<std::uint16_t>::max()){
                        throw sea::sea_errors_handling::RUNTIME_EXECUTION("[RUNTIME EXECUTION] Le champ '" + field.name + "' n'est pas dans la plage des tailles d'un SMALLINT Non Signé.");
                    }
                    ...
                    if(value.get<std::int16_t>() < std::numeric_limits<std::int16_t>::min() ||  value.get<std::int16_t>() > std::numeric_limits<std::int16_t>::max()){
                        throw sea::sea_errors_handling::RUNTIME_EXECUTION("[RUNTIME EXECUTION] Le champ '" + field.name + "' n'est pas dans la plage des tailles d'un SMALLINT.");
                    }
...

                {
                    ...
                    if(unsignedvalue){
                        if(value.get<std::uint32_t>() < std::numeric_limits<std::uint32_t>::min() ||  value.get<std::uint32_t>() > std::numeric_limits<std::uint32_t>::max()){
                            throw sea::sea_errors_handling::RUNTIME_EXECUTION("[RUNTIME EXECUTION] Le champ '" + field.name + "' n'est pas dans la plage des tailles d'un INT Non Signé.");
                        }
                        ...
                        if(value.get<std::int32_t>() < std::numeric_limits<std::int32_t>::min() ||  value.get<std::int32_t>() > std::numeric_limits<std::int32_t>::max()){
                            throw sea::sea_errors_handling::RUNTIME_EXECUTION("[RUNTIME EXECUTION] Le champ '" + field.name + "' n'est pas dans la plage des tailles d'un INT.");
                        }
            ...
            case sea::domain::FieldType::Float: {
                if (!value.is_number()) {
                    throw sea::sea_errors_handling::RUNTIME_EXECUTION("[RUNTIME EXECUTION] Le champ '" + field.name + "' doit etre un nombre.");
                }
            ...
            case sea::domain::FieldType::Bool: {
                if (!value.is_boolean()) {
                    throw sea::sea_errors_handling::RUNTIME_EXECUTION("[RUNTIME EXECUTION] Le champ '" + field.name + "' doit etre un booleen.");
                }
            ...
            case sea::domain::FieldType::Binary: {
                if (!value.is_string()) {
                    throw sea::sea_errors_handling::RUNTIME_EXECUTION(
                        "[RUNTIME EXECUTION] Le champ '" + field.name + "' doit etre une chaine base64."
                        );
                }
