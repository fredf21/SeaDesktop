...
nlohmann::json OpenApiGenerator::generate(
    ...
    doc["info"] = {
        ...
        {"version", "1.0.0"},
        {"description", "OpenAPI documentation automatically generated by SeaDesktop"}
    };
        ...
        for (const auto& relation : entity.relations) {
            ...
            if (relation.kind == RelationKind::HasMany) {
                ...
                json with_children_op = {
                    ...
                    {"responses", {
                                      {"200", {
                                                  {"description", entity.name + " with its " + relation.name}
                                              }},
                                      {"401", {{"description", "Unauthenticated"}}},
                                      {"404", {{"description", "Not found"}}}
                                  }}
    ...
    const std::string& operation_name) const
{
    ...
    if (spec->requires_resource()) {
        oss << "**Strategy**: Resource-aware (after DB load) - slow path\n\n";
    } else {
        oss << "**Strategy**: Subject-only - fast path\n\n";
    }
    ...
    describe_condition = [&](const PolicyCondition& cond, const std::string& indent) {
        ...
        case PolicyConditionType::Any:
            oss << indent << "- **OR** (at least one condition):\n";
            for (const auto& child : cond.children()) {
                describe_condition(child, indent + "    ");
            }
            break;
...
OpenApiGenerator::make_page_envelope_schema(const domain::Entity& entity) const
{
    json schema = {
        {"type", "object"},
        {"description", "Paginated response (page-based mode) for " + entity.name},
        {"properties", {
                           {"items", {
                                         {"type", "array"},
                           ...
                           {"sort",        {{"type", "string"},  {"example", "created_at:desc"},
                                     {"description", "Active sort field and direction (optional)"}}}
                       }},
...
OpenApiGenerator::make_offset_envelope_schema(const domain::Entity& entity) const
{
    json schema = {
        {"type", "object"},
        {"description", "Paginated response (offset/limit mode) for " + entity.name},
        {"properties", {
                           {"items", {
                                         {"type", "array"},
                                         {"items", {{"$ref", "#/components/schemas/" + entity.name}}}
...
OpenApiGenerator::make_cursor_envelope_schema(const domain::Entity& entity) const
{
    json schema = {
        {"type", "object"},
        {"description", "Paginated response (cursor mode) for " + entity.name},
        {"properties", {
                           {"items", {
                                         {"type", "array"},
                                         {"items", {{"$ref", "#/components/schemas/" + entity.name}}}
                           ...
                           {"next_cursor", {{"type", "string"},
                                            {"description", "Token for the next page (absent at the end of the list)"},
                                            {"example", "u-123"}}}
                       }},
    ...
    const domain::Service& service) const
{
    for (const auto& route : route_definitions) {
        ...
        if (info.mode == "page") {
            ...
            if (!cfg.sortable_fields.empty()) {
                ...
                parameters.push_back({
                    ...
                    {"schema", {{"type", "string"}}},
                    {"description", "Sort 'field:asc|desc'. Allowed fields: " +
            ...
            parameters.push_back({
                ...
                {"description", "Maximum number of items (max " +
            ...
            parameters.push_back({
                ...
                {"schema", {{"type", "string"}}},
                {"description", "Cursor from the previous page (empty for the first page)"}
            });
            parameters.push_back({
                {"name", "limit"}, {"in", "query"}, {"required", false},
                {"schema", {{"type", "integer"},
        ...
        const std::string summary =
            "List " + envelope_entity->name + " (paginated " + mode_label + ")";
