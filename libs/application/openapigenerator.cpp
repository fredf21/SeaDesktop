...
nlohmann::json OpenApiGenerator::generate(const domain::Service &service, const std::vector<RouteDefinition> &route_definitions) const
{
...
bool OpenApiGenerator::entity_requires_auth(const domain::Service &service, const std::string &entity_name) const
{
    for (const auto& entity : service.schema.entities) {
...
OpenApiGenerator::json OpenApiGenerator::make_entity_schema(const domain::Entity &entity) const
{
    json schema;
    schema["type"] = "object";
...
OpenApiGenerator::json OpenApiGenerator::make_entity_input_schema(const domain::Entity &entity) const
{
    json schema;
    schema["type"] = "object";
    schema["properties"] = json::object();
...
OpenApiGenerator::json OpenApiGenerator::field_to_openapi_schema(const domain::Field &field) const
{
    switch (field.type) {
...
void OpenApiGenerator::add_crud_path(json &paths, const RouteDefinition &route, const domain::Service &service) const
{
    ...
    entity_plural[1] = static_cast<char>(std::tolower(static_cast<unsigned char>(entity_plural[1])));
    entity_plural += "s";
...
void OpenApiGenerator::add_relation_paths(json &paths, const domain::Service &service) const
{
    for (const auto& entity : service.schema.entities) {
        const bool requires_auth = entity.options.enable_auth;

        std::string parent_path = "/" + entity.name;
        parent_path[1] = static_cast<char>(std::tolower(static_cast<unsigned char>(parent_path[1])));
        parent_path += "s";
        ...
        for (const auto& relation : entity.relations) {
            ...
            if (relation.kind == sea::domain::RelationKind::HasMany) {
                ...
                child_path[1] = static_cast<char>(std::tolower(static_cast<unsigned char>(child_path[1])));
                child_path += "s";

                std::string parent_name = entity.name;
                parent_name[0] = static_cast<char>(std::tolower(static_cast<unsigned char>(parent_name[0])));

                const std::string by_id_path =
                    child_path + "/filter/with_" + parent_name + "/{id}";

                json op = {
                    {"summary", "List related " + relation.target_entity},
                    {"parameters", json::array({
                                       {
                                           {"name", "id"},
                                           {"in", "path"},
