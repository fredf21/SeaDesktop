...
// ─────────────────────────────────────────────────────────────────────
// DetachManyToManyHandler
//
// Supprime une association dans une table pivot many-to-many.
//
// Route attendue : DELETE /<source_entity_lower>s/{id}/<relation_name>/{target_id}
//
// Exemple : DELETE /articles/{id}/tags/{target_id}
//   -> supprime (article_id=id, tag_id=target_id) de article_tags
//
// Comportement :
//   - 404 si la ressource source (id) n'existe pas
//   - 404 si la ressource cible (target_id) n'existe pas
//   - 404 si l'association n'existait pas
//   - 204 No Content en cas de succes
//   - 403 si l'ABAC refuse l'acces a la ressource source ou cible
//
// L'ABAC est verifie sur les deux extremites de la relation.
// ─────────────────────────────────────────────────────────────────────
class DetachManyToManyHandler final : public seastar::httpd::handler_base {
public:
    DetachManyToManyHandler(
